...
UPDATE RESUMO_MENSAL_VENDA
       SET RMV_VL_TOTAL = 200000
       WHERE CLI_IN_CODIGO = 10 AND PRO_IN_CODIGO = 9;

SELECT * FROM CLIENTE WHERE CLI_IN_CODIGO = 10;
...
SELECT pro_in_codigo, count(pro_in_codigo), sum(rmv.rmv_vl_total) FROM RESUMO_MENSAL_VENDA RMV
       WHERE RMV.CLI_IN_CODIGO = 20
       GROUP BY PRO_IN_CODIGO;
...
-----
  SELECT pro_in_codigo, count(pro_in_codigo) from item_nota_fiscal_venda group by pro_in_codigo order by pro_in_codigo;
  SELECT pro_in_codigo, count(pro_in_codigo)teste from item_nota_fiscal_venda group by pro_in_codigo order by pro_in_codigo;

      SELECT pro_in_codigo, COUNT(pro_in_codigo)quantidade 
           FROM item_nota_fiscal_venda infv, nota_fiscal_venda nfv
           WHERE nfv.nfv_in_numero = infv.nfv_in_numero
                 AND nfv.cli_in_codigo = 10
           GROUP BY pro_in_codigo 
           ORDER BY pro_in_codigo;
